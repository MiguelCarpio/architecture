...
Unlike SR-IOV or mediated-device (mdev / vGPU) setups, full-device passthrough
assigns the entire physical GPU to a single guest. `nouveau` and `nvidia` are
...
native NVIDIA driver is installed inside the guest VM only.
...
openstack --os-compute-api-version 2.86 flavor create rtx-pro-6000-1 \
...
Install the standard native NVIDIA driver in the guest.
